  ...
  return (
    <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        ...
        <ScrollView style={styles.listContainer}>
          {selectedExercises.map((exercise) => (
            <View key={exercise.id} style={styles.exerciseRow}>
              <TextInput style={styles.input} value={exercise.name} onChangeText={(text) => updateExercise(exercise, 'name', text)} placeholder="Exercise name" placeholderTextColor="#aaa" />
              <TextInput style={styles.input} keyboardType="numeric" value={exercise.load.toString()} onChangeText={(text) => updateExercise(exercise, 'load', parseFloat(text) || 0)} placeholder="Load (kg)" placeholderTextColor="#aaa" />
              <TextInput style={styles.input} keyboardType="numeric" value={exercise.repetitions.toString()} onChangeText={(text) => updateExercise(exercise, 'repetitions', parseInt(text) || 0)} placeholder="Repetitions" placeholderTextColor="#aaa" />
        ...
        <TextInput style={styles.input} value={sessionDate} onChangeText={setSessionDate} placeholder="Enter session date (e.g., 2025-02-09)" placeholderTextColor="#aaa" />
